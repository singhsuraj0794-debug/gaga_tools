...
def main():
    ...
    for step in flow_results:
        if step.get("step", "").endswith("session_recording") and step.get("video_path"):
            step_name = step["step"]
            platform = next((p for p in ("mweb", "web") if step_name.startswith(f"{p}_")), "unknown")
    ...
    for step in flow_results:
        ...
        platform = next((p for p in ("mweb", "web") if step_name.startswith(f"{p}_")), None)
        details = {
        ...
        store.store_flow_step("happy_flow", step_name, duration, step_status, (error or failure_reason or detail[:200] if detail else error) if step_status != "pass" else None, details, issue_type=step.get("issue_type"))
