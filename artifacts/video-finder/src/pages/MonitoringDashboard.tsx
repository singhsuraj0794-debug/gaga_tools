...
export default function MonitoringDashboard() {
  ...
  useEffect(() => {
    ...
    if (!supabaseUrl || !supabaseKey) {
      setError("VITE_SUPABASE_URL and VITE_SUPABASE_KEY must be set in environment");
      setLoading(false);
      return;
    }
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-red-500 text-xs mt-2">
                    ...
                    <code className="bg-red-100 px-1 rounded">VITE_SUPABASE_KEY</code> in your .env file.
                  </p>
                </div>
              </div>
            </CardContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {Object.entries(byPage).map(([page, pageRuns]) => {
            ...
            const pageStatus = pageLatest.some((r) => r.status === "fail")
              ? "fail"
              : pageLatest.some((r) => r.status === "degraded")
              ? "degraded"
              : "pass";
            ...
            return (
              <Card key={page} className="border-l-4 transition-shadow hover:shadow-md" style={{
                borderLeftColor:
                  pageStatus === "fail" ? "#ef4444" : pageStatus === "degraded" ? "#eab308" : "#22c55e",
              }}>
